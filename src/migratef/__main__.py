...
def _collect_wait_candidates(
    base_dir: Path,
    source_paths: List[str],
    already_dir: Path,
    wait_dir: Path,
) -> List[str]:
    """Collect sibling paths that should be moved to wait in auto mode."""
    selected = {Path(p).resolve() for p in source_paths}
    candidates: List[str] = []
    for item in base_dir.iterdir():
        if item.resolve() in selected:
            continue
        if item.resolve() in {already_dir.resolve(), wait_dir.resolve()}:
            continue
...
def migrate(
    ...
    if classify in {"auto", "only"}:
        if not direct:
            logger.error("错误: --classify 仅支持 --direct 模式")
            raise typer.Exit(code=1)
        ...
        if target:
            ...
            if not base_dir:
                logger.error("错误: --classify 需要所有源路径位于同一目录，或显式提供 --target")
                raise typer.Exit(code=1)
        ...
        if classify == "auto":
            ...
            wait_candidates = _collect_wait_candidates(
