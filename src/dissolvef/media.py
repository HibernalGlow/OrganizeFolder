...
def release_single_media_folder(path, exclude_keywords=None, logger=None, preview=False):
    """
    ...
    if logger is None and not preview:  # 只有在没有日志记录器的情况下才显示状态
        status.start()
    ...
    try:
        for root, dirs, files in os.walk(path, topdown=False):
            ...
            try:
                ...
                if len(media_files) == 1 and len(files) == 1 and len(dirs) == 0:
                    ...
                    if logger:
                        logger.info(f"\n找到符合条件的文件夹: {root}")
                        logger.info(f"- 单个媒体文件({media_type}): {media_file.name}")
                    else:
                    ...
                    if len(media_files) > 0 and (logger or preview):
                        if logger:
        ...
        # 确保状态指示器被停止
        if logger is None and not preview and status.started:
            status.stop()
